...
"#
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        if !path.exists() {
            return Ok(Config {
                default_video_format: Some("mp4".into()),
        ...
        let content =
            fs::read_to_string(path).with_context(|| format!("無法讀取設定檔: {:?}", path))?;
    ...
    pub fn save(&self, path: &Path) -> Result<()> {
        let content = toml::to_string_pretty(self).context("序列化設定資料失敗")?;

        fs::write(path, content).with_context(|| format!("無法寫入設定檔至: {:?}", path))?;
