...
pub fn process_external_subtitles(tmp_dir: &Path, ts: &str, final_name: &str, target_dir: &Path, media_type: u8) {
...
pub fn merge_subs_and_danmaku(tmp_dir: &Path, ts: &str, video_path: &Path, final_path: &Path) -> bool {
    ...
    if let Ok(entries) = fs::read_dir(tmp_dir) {
        for entry in entries.flatten() {
            ...
            if name.starts_with(&format!("tmp_{}", ts)) && (name.ends_with(".ass") || name.ends_with(".clean.vtt")) {
                ...
                sub_files.push((entry.path(), display_title.to_string(), iso_lang.to_string()));
            }
        }
    }

    if sub_files.is_empty() { return false; }
...
pub fn get_video_resolution(path: &Path) -> Option<String> {
    let output = Command::new("ffprobe")
        .args(["-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height", "-of", "csv=s=x:p=0", path.to_str().unwrap()])
        .output().ok()?;
    Some(String::from_utf8_lossy(&output.stdout).trim().to_string())
}

/// 終極大掃除
pub fn cleanup_tmps(tmp_dir: &Path, ts: &str) {
    let prefix = format!("tmp_{}", ts);
    if let Ok(entries) = fs::read_dir(tmp_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                // 🎯 修復 2：嚴格限制只能刪除以 tmp_ 為開頭的檔案！
                // 絕對不會誤傷已經改名為最終檔名的成品
